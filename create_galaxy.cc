...
int Galprop::create_galaxy()  //AWS20050816
{
	...
	if(galdef.n_spatial_dimensions==3) {
		...
		// ECC24012016
		// If no high resolution inner region used, just use teh old routine.
		if (   (galdef.x_min_inner==galdef.x_max_inner)
		       && (galdef.y_min_inner==galdef.y_max_inner)
		       && (galdef.z_min_inner==galdef.z_max_inner)) {
			...
			galaxy.init(galdef.x_min,  galdef.x_max, galdef.dx,
			            galdef.y_min,  galdef.y_max, galdef.dy,
			            galdef.z_min,  galdef.z_max, galdef.dz,
			            galdef.x_min_inner,  galdef.x_max_inner, galdef.dx_inner,
			            galdef.y_min_inner,  galdef.y_max_inner, galdef.dy_inner,
			            galdef.z_min_inner,  galdef.z_max_inner, galdef.dz_inner);
		}
	}
		...
		for (int ir = 0; ir < galaxy.n_rgrid; ++ir) {
			...
			for (int iz = 0; iz < galaxy.n_zgrid; ++iz) {
				...
				if (galdef.synchrotron>=2) { //AWS20131003  (was ==2)
					...
					galaxy.B_field.d2[ir][iz].s[0] = B_field_3D_model_tot(galdef.B_field_name, galdef.B_field_parameters, galaxy.r[ir], galaxy.z[iz]);     // Gauss->Tesla
			...
			for (int iy = 0; iy < galaxy.n_ygrid; ++iy) {
				...
				for (int iz = 0; iz < galaxy.n_zgrid; ++iz) {
					...
					if ( galdef.synchrotron>=2) { //AWS20131003 (was ==2)
						...
						galaxy.B_field.d3[ix][iy][iz].s[0] = B_field_3D_model_tot(galdef.B_field_name, galdef.B_field_parameters, galaxy.x[ix], galaxy.y[iy], galaxy.z[iz],B_field_debug);   //AWS20140526
