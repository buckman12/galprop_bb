...
typedef struct {
	double val;
	double err;
} gsl_sf_result; // structure used by GSL routines
...
//   zscale=0.200;  //nominal value AWS20131108 replaced by parameters[0]
	...
	if (21 == srcModel) { //arbitrary parametrization
		zscale=parameters[0]; //AWS20131108
		alpha= parameters[1];
		beta = parameters[2];
		rmax = parameters[3];
...
//    zscale=0.200;  //nominal value AWS20131108 replaced by parameters[0]
		...
		// Add gaussian source at the GC.  For sigma=200 pc, this implies a normalization of
		// 1% of SNR rate per unit source_parameter_5 within 300 pc
		// Don't overflow the exponential at large radius.
		if(r<4) {
			result +=  74.574497247* roff * exp( - r_3d*r_3d/ (rconst*rconst) );
		}

		if (r >= rmax)  result = 0.0;
	}
	if (22== srcModel) { //SNR Green 1508.02931
		alpha=1.49;
		beta=4.6;
		ro=8.5;
		zscale=0.200;  // nominal value;
	...
	if (30 == srcModel) { //M82_a BJB20160607
		double zscale= parameters[0];
