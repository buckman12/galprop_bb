	...
	if (2 == particle.n_spatial_dimensions) { // ==== 2D ====
		...
		*/
		...
		if (galdef.convection==4) {
			...
			for (ir = 0; ir < particle.n_rgrid; ++ir) { // numerator = abs convection velocity in cm s^-1
				for (iz = 0; iz < particle.n_zgrid; ++iz) { // numerator = abs convection velocity in cm s^-1

					/* Calculate the grid spacing along each direction.*/
					double dr_up, dr_mid, dr_down;
					double dz_up, dz_mid, dz_down;
					//check bounds in case we are at the boundary.  If we are, just use full grid spacing
					if (ir==(particle.n_rgrid-1) || ir==0) {
						dr_up   = particle.dr;
						dr_mid  = particle.dr;
						dr_down = particle.dr;
					}
						...
						dr_up   = particle.r[ir+1]-particle.r[ir];
						dr_down = particle.r[ir]-particle.r[ir-1];
						// average the other two for the middle coeff.  This doesn't quite seem right..
						dr_mid  = .5*(dr_up + dr_down);

					}

					// z-axis
					if (iz==(particle.n_zgrid-1) || iz==0) {
						dz_up   = particle.dz;
						dz_mid  = particle.dz;
						dz_down = particle.dz;
					} else {
						dz_up   = particle.z[iz+1]-particle.z[iz];
						dz_down = particle.z[iz]-particle.z[iz-1];
						dz_mid  = .5*(dz_up + dz_down); // average the other two for the middle coeff.  This doesn't quite seem right..
					}

					double ar1 = (particle.r[ir] > 0.) ? fabs(particle.v_conv_x.d2[ir-1][iz].s[0])*1.e5/dr_down/kpc2cm: 0.;
					double ar2 =                         fabs(particle.v_conv_x.d2[ir  ][iz].s[0])*1.e5/dr_mid/kpc2cm;
					double ar3 = (particle.r[ir] < 0.) ? fabs(particle.v_conv_x.d2[ir+1][iz].s[0])*1.e5/dr_up/kpc2cm: 0.;

					double az1 = (particle.z[iz] > 0.) ? fabs(particle.v_conv_z.d2[ir][iz-1].s[0])*1.e5/dz_down/kpc2cm: 0.;
					double az2 =                         fabs(particle.v_conv_z.d2[ir][iz  ].s[0])*1.e5/dz_mid/kpc2cm;
					double az3 = (particle.z[iz] < 0.) ? fabs(particle.v_conv_z.d2[ir][iz+1].s[0])*1.e5/dz_up/kpc2cm: 0.;

					// Galactic center is a special case.
					if ( particle.z[iz]==0 && particle.r[ir]==0) {

						//ar1 = fabs(particle.v_conv_x.d2[ir-1][iz].s[0])*1.e5/dr_down/kpc2cm;
						ar1 = 0; // no propagation through GC in 2D here, just outflow.
						ar2 = fabs(particle.v_conv_x.d2[ir][iz].s[0])*1.e5/dr_mid/kpc2cm;
						ar3 = fabs(particle.v_conv_x.d2[ir+1][iz].s[0])*1.e5/dr_up/kpc2cm;


						az1 = fabs(particle.v_conv_z.d2[ir][iz-1].s[0])*1.e5/dz_down/kpc2cm;
						az2 = fabs(particle.v_conv_z.d2[ir][iz].s[0])*1.e5/dz_mid/kpc2cm;
						az3 = fabs(particle.v_conv_z.d2[ir][iz+1].s[0])*1.e5/dz_up/kpc2cm;
					}
					...
					for (ip = 0; ip < particle.n_pgrid-1; ++ip) {
						alpha1_r.d2[ir][iz].s[ip] += ar1;
						alpha1_z.d2[ir][iz].s[ip] += az1;

						alpha2_r.d2[ir][iz].s[ip] += ar2;
						alpha2_z.d2[ir][iz].s[ip] += az2;

						alpha3_r.d2[ir][iz].s[ip] += ar3;
						alpha3_z.d2[ir][iz].s[ip] += az3;
		...
		for (timestep_mode = 1; timestep_mode <= 2; ++timestep_mode) {
			...
			while ((1 == start) ||
				...
				if (1 == timestep_mode) {
					...
					buf.str("");
					buf<<"timestep_mode 1: using solution method "<<method<< " timestep="<<dt/year2sec<< " yr";
					INFO(buf);
				}

				if (2 == timestep_mode) {
					if(galdef.solution_method== 1) {
						method=1;            //AWS20110131
					}
					if(galdef.solution_method== 2) {
						method=2;    //AWS20110131
						turbo=0;
					}
					if(galdef.solution_method==21) {
						method=2;    //AWS20110131
					...
					if(method==1) {
						buf.str("");
						buf<<"timestep_mode 2: using  method "<<method<<" Crank-Nicolson"
						   << " timestep="<<dt/year2sec<< " yr";
					}
					if(method==2) {
						buf.str("");
						buf<<"timestep_mode 2: using  method "<<method<<" fully time explicit, "
		...
		for (ix = 0; ix < particle.n_xgrid; ++ix) {
			...
			for (iy = 0; iy < particle.n_ygrid; ++iy) {
				...
				for (iz = 0; iz < particle.n_zgrid; ++iz) {
					...
					/* Calculate the grid spacing along each direction.  This is only used if non-uniform grid spacing used..*/
					double dx_up, dx_down;
					double dy_up, dy_down;
					double dz_up, dz_down;
					//check bounds in case we are at the boundary.  If we are, just use full grid spacing
					if (ix==(particle.n_xgrid-1) || ix==0) {
						dx_up   = particle.dx;
						dx_down = particle.dx;
					} else {
						dx_up   = particle.x[ix+1]-particle.x[ix];
						dx_down = particle.x[ix]-particle.x[ix-1];
						// average the other two for the middle coeff.  This doesn't quite seem right..
					}

					// y-axis
					if (iy==(particle.n_ygrid-1) || iy==0) {
						dy_up   = particle.dy;
						dy_down = particle.dy;
					} else {
						dy_up   = particle.y[iy+1]-particle.y[iy];
						dy_down = particle.y[iy]-particle.y[iy-1];
					}
					...
					for (ip = 0; ip < particle.n_pgrid; ++ip) {
						...
						// ECC 02/12/2016 Adjust for non-uniform grid spacing.
						// alpha1_x.d3[ix][iy][iz].s[ip]
						//   = particle.Dxx.d3[ix][iy][iz].s[ip]*pow(dx_down, -2.);
						// alpha1_y.d3[ix][iy][iz].s[ip]
						//   = particle.Dxx.d3[ix][iy][iz].s[ip]*pow(dy_down, -2.);
						// alpha1_z.d3[ix][iy][iz].s[ip]
						//   = particle.Dzz.d3[ix][iy][iz].s[ip]*pow(dz_down, -2.);

						// alpha2_x.d3[ix][iy][iz].s[ip]
						//   =   particle.Dxx.d3[ix][iy][iz].s[ip]*pow(dx_down, -2.)
						//     + particle.Dxx.d3[ix][iy][iz].s[ip]*pow(dx_up, -2.);
						// alpha2_y.d3[ix][iy][iz].s[ip]
						//   = particle.Dxx.d3[ix][iy][iz].s[ip]*pow(dy_down, -2.)
						//     + particle.Dxx.d3[ix][iy][iz].s[ip]*pow(dy_up, -2.);
						// alpha2_z.d3[ix][iy][iz].s[ip]
						//   = particle.Dzz.d3[ix][iy][iz].s[ip]*pow(dz_down, -2.)
						//     + particle.Dzz.d3[ix][iy][iz].s[ip]*pow(dz_up, -2.);
						...
						/// THIS IS THE OLD WAY.  REQUIRES UNIFORM GRID SPACING
						alpha1_x.d3[ix][iy][iz].s[ip]
		...
		if (galdef.convection==1) {
			...
			for (iz = 0; iz < particle.n_zgrid; ++iz) { // numerator = abs convection velocity in cm s^-1
				...
				*/
				...
				// should be exactly the same for v0_conv=0
				// ECC NOTE THIS NEEDs TO BE FIXED FOR NON_UNIFORM GRID SPACING....
			...
			for (ix = 0; ix < particle.n_xgrid; ++ix) { // numerator = abs convection velocity in cm s^-1
				for (iy = 0; iy < particle.n_ygrid; ++iy) { // numerator = abs convection velocity in cm s^-1
					for (iz = 0; iz < particle.n_zgrid; ++iz) { // numerator = abs convection velocity in cm s^-1
						...
						/* Calculate the grid spacing along each direction.*/
						double dx_up, dx_mid, dx_down;
						double dy_up, dy_mid, dy_down;
						double dz_up, dz_mid, dz_down;
						//check bounds in case we are at the boundary.  If we are, just use full grid spacing
						if (ix==(particle.n_xgrid-1) || ix==0) {
							dx_up   = particle.dx;
						...
						if (iy==(particle.n_ygrid-1) || iy==0) {
							...
							dy_up   = particle.y[iy+1]-particle.y[iy];
							dy_down = particle.y[iy]-particle.y[iy-1];
							dy_mid  = .5*(dy_up + dy_down); // average the other two for the middle coeff.  This doesn't quite seem right..
						}
						...
						double ax1 = (particle.x[ix] > 0.) ? fabs(particle.v_conv_x.d3[ix-1][iy][iz].s[0])*1.e5/dx_down/kpc2cm: 0.;
						double ax2 =                         fabs(particle.v_conv_x.d3[ix  ][iy][iz].s[0])*1.e5/dx_mid/kpc2cm;
						double ax3 = (particle.x[ix] < 0.) ? fabs(particle.v_conv_x.d3[ix+1][iy][iz].s[0])*1.e5/dx_up/kpc2cm: 0.;

						double ay1 = (particle.y[iy] > 0.) ? fabs(particle.v_conv_y.d3[ix][iy-1][iz].s[0])*1.e5/dy_down/kpc2cm: 0.;
						double ay2 =                         fabs(particle.v_conv_y.d3[ix][iy  ][iz].s[0])*1.e5/dy_mid/kpc2cm;
						double ay3 = (particle.y[iy] < 0.) ? fabs(particle.v_conv_y.d3[ix][iy+1][iz].s[0])*1.e5/dy_up/kpc2cm: 0.;

						double az1 = (particle.z[iz] > 0.) ? fabs(particle.v_conv_z.d3[ix][iy][iz-1].s[0])*1.e5/dz_down/kpc2cm: 0.;
						double az2 =                         fabs(particle.v_conv_z.d3[ix][iy][iz  ].s[0])*1.e5/dz_mid/kpc2cm;
						double az3 = (particle.z[iz] < 0.) ? fabs(particle.v_conv_z.d3[ix][iy][iz+1].s[0])*1.e5/dz_up/kpc2cm: 0.;

						// Galactic center is a special case.
						if ( particle.z[iz]==0 && particle.y[iy]==0 && particle.x[ix]==0) {
							ax1 = fabs(particle.v_conv_x.d3[ix-1][iy][iz].s[0])*1.e5/dx_down/kpc2cm;
							ax2 = fabs(particle.v_conv_x.d3[ix][iy][iz].s[0])*1.e5/dx_mid/kpc2cm;
							ax3 = fabs(particle.v_conv_x.d3[ix+1][iy][iz].s[0])*1.e5/dx_up/kpc2cm;

							ay1 = fabs(particle.v_conv_x.d3[ix][iy-1][iz].s[0])*1.e5/dy_down/kpc2cm;
							ay2 = fabs(particle.v_conv_x.d3[ix][iy][iz].s[0])*1.e5/dy_mid/kpc2cm;
							ay3 = fabs(particle.v_conv_x.d3[ix][iy+1][iz].s[0])*1.e5/dy_up/kpc2cm;

							az1 = fabs(particle.v_conv_x.d3[ix][iy][iz-1].s[0])*1.e5/dz_down/kpc2cm;
							az2 = fabs(particle.v_conv_x.d3[ix][iy][iz].s[0])*1.e5/dz_mid/kpc2cm;
							az3 = fabs(particle.v_conv_x.d3[ix][iy][iz+1].s[0])*1.e5/dz_up/kpc2cm;
						}
						...
						for (ip = 0; ip < particle.n_pgrid-1; ++ip) {
							...
							// ECC: NOTE WE HAVE NEGELECTED ADIABIATIC LOSSES HERE! \
							// This is ok since there are no significant gradients in the convection velocity
							// alpha_p \propto dV/dx, dV/dy, dV/dz etc... see galprop explanatory suppl. pg 45 for more info.
		...
		for (timestep_mode = 1; timestep_mode <= 2; ++timestep_mode) {
			...
			while ((1 == start) ||
				...
				if (2 == timestep_mode) {
					...
					if(galdef.solution_method== 1) {
						method=1;            //AWS20110201
					}
					if(galdef.solution_method== 2) {
						method=2;    //AWS20110201
						turbo=0;
					}
					if(galdef.solution_method==21) {
						method=2;    //AWS20110201
				...
				for (irept = 1; irept <= nrept; ++irept) {  //AWS20111018
					...
					// i.e. a delta function injection. 	  BJB20160610 removed s_p[9]==0


					/*if (irept>1 && 2==timestep_mode){
