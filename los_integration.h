...
void LOSintegrator<T>::findLocalAnnulus() {
   fiLocalAnnulus = findAnnulus(fabs(fxorig));
}
template <typename T>
...
void LOSintegrator<T>::checkBins() const {
   ...
   if (fdimensions == 2) {
      ...
      double xmax = fxmax > fabs(fxmin) ? fxmax : fabs(fxmin);
      double ymax = fymax > fabs(fymin) ? fymax : fabs(fymin);
...
std::vector< std::vector<T> > LOSintegrator<T>::integrate ( const double l, const double b, const double dl, std::vector<LOSfunction<T>*> funcs) {
    ...
    while ( d + fstepSize < dmax ) {
	  ...
	  double ledge(0);
	  if ( l > 270. )
	     ledge=360 - asin ( fRbins[maxAnn+1]/fabs(fxorig) ) / dtr;
	  else
    	     ledge=asin ( fRbins[minAnn+1]/fabs(fxorig) ) / dtr;
