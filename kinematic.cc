              ...
              double &beta, double &gamma, double &rigidity,int test)
{
    ...
    if(p>=0.0 && Ekin>=0.0) {
        cout<<"kinematic: error:p or Ekin to be evaluated ?"<<endl;    // one of them has to be <0
        return -1;
    }
    if(p<=0.0 && Ekin<=0.0) {
        cout<<"kinematic: error:p or Ekin has to be >0     "<<endl;    // one of them has to be >0
        return -2;
    }
    if(strcmp(species,"nucleus" )!=0 && strcmp(species,"electron")!=0) {
        cout<<"invalid species:"<<species<<endl;    // check for electron or nucleus
        return -1;
    }
