...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(name == "galprop_original") {
		...
		*/
		...
		/*cout<<"galprop original model="<<name<<" Bo="<<Bo<<" rscale="<<rscale<<" zscale="<<zscale
		<<" (x, y, z) = ("<<x<<", "<<y<<", "<<z<<")" << " r=" <<r
		<<" b_field="<<b_field<<endl; */
	...
	if(name == "galprop_original_y") {
		...
		*/
		...
		Bran=b_field;
		Branx=0.0;  // put all field in x-direction (has no significance)
		Brany=Bran;
		Branz=0.0;
...
// if (z<0.){Bhalox=-Bhalox/2.; Bhaloy=-Bhaloy/2.;} // the field in the south is 1/2!!!!
		if (z<0.) {
			Bhalox=-Bhalox;       //AWS20120405 1/2 factor already applied above, applies to ASS only
			Bhaloy=-Bhaloy   ;
		}
...
// iteratively figure out which spiral arm the current coordinates (r, phi) corresponds to.
				...
				if (r_negx > rc_B[7]*CGS_U_kpc) {
					r_negx = r*exp(-1/tan(CGS_U_pi/180.*(90-inc))*(PHI+  CGS_U_pi));
				}
				if (r_negx > rc_B[7]*CGS_U_kpc) {
					r_negx = r*exp(-1/tan(CGS_U_pi/180.*(90-inc))*(PHI+3*CGS_U_pi));
				}
...
// dividing line between region with constant elevation angle, and the interior:
		...
		if (r<rc_X) { // interior region, with varying elevation angle
			...
			if (coords_z==0.) {
				Xtheta=CGS_U_pi/2.;   // to avoid some NaN
			}
...
//============================================================================
	...
	if(name == "m82_simple_a") {
		// BJB20160531
		// zeroth-order: Bo=5.0e-5 G rscale=0.2 kpc zscale=0.05 kpc b_power=+2
